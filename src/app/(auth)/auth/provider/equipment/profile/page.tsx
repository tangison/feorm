...
export default function ProviderEquipmentProfilePage() {
  ...
  const handleContinue = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setSelectedRole("provider_equipment");
      ...
      setUser((prev: any) =>
        prev
          ? { ...prev, name, phone, region, role: "provider_equipment" as const }
          : null
      );
      router.push("/auth/provider/equipment/assets");
    } catch {
      router.push("/auth/provider/equipment/assets");
    }
    setLoading(false);
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <input
              ...
              placeholder="Namib Agricultural Equipment"
              className="w-full bg-transparent outline-none text-lg text-white placeholder-white/30"
            />
          </div>
          ...
          >
            {loading ? "Saving..." : "Add My Equipment"}
            <ArrowRight size={14} />
          </button>
        </div>
      </div>
